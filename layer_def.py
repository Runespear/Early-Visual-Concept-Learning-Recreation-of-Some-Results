...
def conv_layer(inputs, kernel_size, stride, num_features, idx, linear=False):
    with tf.variable_scope('{0}_conv'.format(idx)) as scope:
        ...
        biases = _variable_on_cpu(
            'biases', [num_features], tf.constant_initializer(0.01))
...
def transpose_conv_layer(inputs, kernel_size, stride, num_features, idx, linear=False):
    with tf.variable_scope('{0}_trans_conv'.format(idx)) as scope:
        ...
        if linear:
            return conv_biased
        conv_rect = tf.nn.elu(
            conv_biased, name='{0}_transpose_conv'.format(idx))
        return conv_rect
...
def fc_layer(inputs, hiddens, idx, flat=False, linear=False):
    with tf.variable_scope('{0}_fc'.format(idx)) as scope:
        ...
        weights = _variable_with_weight_decay(
            'weights', shape=[dim, hiddens],
            stddev=FLAGS.weight_init, wd=FLAGS.weight_decay)
        biases = _variable_on_cpu(
            'biases', [hiddens], tf.constant_initializer(FLAGS.weight_init))
        if linear:
            return tf.add(tf.matmul(inputs_processed, weights),
                          biases, name=str(idx) + '_fc')
