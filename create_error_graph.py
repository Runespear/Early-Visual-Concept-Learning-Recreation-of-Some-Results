...
def test_loss(network_type):
    ...
    # calc loss stuff
    loss_vae, loss_reconstruction, loss, train_op = ls.loss(
        mean, stddev, x, x_prime)
    ...
    ckpt = tf.train.get_checkpoint_state(
        './checkpoints/train_store_' + network_type)
    ...
    for step in xrange(FLAGS.num_runs):
        ...
        loss_reconstruction_r = loss_reconstruction_r + \
            np.sum(sess.run([loss_reconstruction],
                            feed_dict={x: dat})) / FLAGS.batch_size
        if np.sum(sess.run([loss_reconstruction],
                           feed_dict={x: dat})) / FLAGS.batch_size < 1.0:
            print(np.sum(sess.run([loss_reconstruction],
                                  feed_dict={x: dat})) / FLAGS.batch_size)
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    loss_model_conv_num_balls_1[0] = test_loss(
        "model_conv_num_balls_1_beta_0.1")
    loss_model_conv_num_balls_1[1] = test_loss(
        "model_conv_num_balls_1_beta_0.5")
    loss_model_conv_num_balls_1[2] = test_loss(
        "model_conv_num_balls_1_beta_1.0")

    loss_model_conv_num_balls_2 = np.zeros(3)
    loss_model_conv_num_balls_2[0] = test_loss(
        "model_conv_num_balls_2_beta_0.1")
    loss_model_conv_num_balls_2[1] = test_loss(
        "model_conv_num_balls_2_beta_0.5")
    loss_model_conv_num_balls_2[2] = test_loss(
        "model_conv_num_balls_2_beta_1.0")

    loss_model_fully_connected_num_balls_1 = np.zeros(3)
    loss_model_fully_connected_num_balls_1[0] = test_loss(
        "model_fully_connected_num_balls_1_beta_0.1")
    loss_model_fully_connected_num_balls_1[1] = test_loss(
        "model_fully_connected_num_balls_1_beta_0.5")
    loss_model_fully_connected_num_balls_1[2] = test_loss(
        "model_fully_connected_num_balls_1_beta_1.0")

    loss_model_fully_connected_num_balls_2 = np.zeros(3)
    loss_model_fully_connected_num_balls_2[0] = test_loss(
        "model_fully_connected_num_balls_2_beta_0.1")
    loss_model_fully_connected_num_balls_2[1] = test_loss(
        "model_fully_connected_num_balls_2_beta_0.5")
    loss_model_fully_connected_num_balls_2[2] = test_loss(
        "model_fully_connected_num_balls_2_beta_1.0")

    loss_model_all_conv_num_balls_1 = np.zeros(3)
    loss_model_all_conv_num_balls_1[0] = test_loss(
        "model_all_conv_num_balls_1_beta_0.1")
    loss_model_all_conv_num_balls_1[1] = test_loss(
        "model_all_conv_num_balls_1_beta_0.5")
    loss_model_all_conv_num_balls_1[2] = test_loss(
        "model_all_conv_num_balls_1_beta_1.0")

    loss_model_all_conv_num_balls_2 = np.zeros(3)
    loss_model_all_conv_num_balls_2[0] = test_loss(
        "model_all_conv_num_balls_2_beta_0.1")
    loss_model_all_conv_num_balls_2[1] = test_loss(
        "model_all_conv_num_balls_2_beta_0.5")
    loss_model_all_conv_num_balls_2[2] = test_loss(
        "model_all_conv_num_balls_2_beta_1.0")
    ...
    plt.scatter(beta, loss_model_all_conv_num_balls_1,
                label="all_conv", color='red')
    plt.scatter(beta, loss_model_fully_connected_num_balls_1,
                label="fully_connected", color='green')
    ...
    plt.scatter(beta, loss_model_all_conv_num_balls_2,
                label="all_conv", color='red')
    plt.scatter(beta, loss_model_fully_connected_num_balls_2,
                label="fully_connected", color='green')
